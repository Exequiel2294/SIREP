...
@section('js')
    ...
    <script>
        ...
        $(function(){
            ...
            $('#modal-people').on('hidden.bs.modal', function () {
                $('#data-table').DataTable().ajax.reload(null, false);
                $('#data-table-people').DataTable().ajax.reload(null, false);
            });
            /* ACTION TO CLOSE MODAL */
        })
        /*FIN Fecha*/
        ...
        $(document).ready(function(){  
            $("#data-table").DataTable({
                ...
                buttons: [
                    ...
                    {
                        extend: 'csvHtml5',
                        title: 'Listado Capacitacion '+moment().local().format('DD/MM/YYYY'),
                        filename: 'ListadoCapacitacion'+moment().local().format('DD/MM/YYYY'),
                        className: 'buttonsToHide'
                    }, 
                    {
                        extend: 'excelHtml5',
                ...
                columns: [
                    ...
                    {data:'hora', name:'hora'},
                    {data:'tema', name:'tema'},
                    {data:'capacitador', name:'capacitador'},
                    {data:'area', name:'area'},
                    {data:'lugar', name:'lugar'},
                    {data:'duracion', name:'duracion'},
                    {data:'asist', name:'asist', orderable: false,searchable: false, width:'30px'},
                    {data:'evaluated', name:'evaluated', orderable: false,searchable: false, width:'30px'},
        ...
        /* ADD BUTTON*/
        $('#add').click(function () { 
            //Modificar para que se pueda agregar una capacitacion
            $('#form-button').val(1);     
            $('#modal-title').html('Cargar Capacitacion'); 
        ...
        /* DEPLOY MODAL WITH SELECT PEOPLE*/
        $(document).on('click','.view',function(){
            var employee=$(this).data('id');
            $('#modal-people').modal('show');
            /*Table de empleados*/
            var t = $("#data-table-people").DataTable({    
                ...
                columns: [
                    {data:'DNI', name:'DNI' , visible: false, orderable: false,searchable: false},
        ...
        /* Agregar Notificacio cuando se desapruebe o apruebe una persona agregar el MansfieldRep.Notification*/
        $(document).on('click','.btn-aprobar',function(){
            var id=$(this).data('id');
           $.ajax({
                url:"{{route('capacitacion.aproved') }}",
                type:"POST",
                dateType:"json",
                data:{
                    id_relacion:id,
                ...
                success:function(dato){
                    $('#data-table-aproved').DataTable().ajax.reload(null,false);
                },
                error:function(){
        ...
        /* FIN BOTON APROBAR/DESAPROBAR */

        /* VER LISTADO DE APROBADOS/DESAPROBADOS BUTTON */
        $(document).on('click', '.aproved', function(){ 
            var id=$(this).data('id');
            /* Tabla de personas a aprobar/desaprobar */
            $('#modal-aproved').modal('show')
            var p = $("#data-table-aproved").DataTable({    
                dom:    "<'row data-search'<'col-sm-12'f>>" +
                        "<'row'<'col-sm-12'tr>>" +
                        "<'row data-buttons'<'col-sm-12'p>>",  
                lengthMenu: [[10, 25], [10, 25]],
        ...
        /* VER APROBADO/DESAPROBADO BUTTON */

        /* CHECK AGREGAR PERSONAS A LA CAPACITACION */
        $(document).on('click','.checkpersonas',function(){
            var data = {
                id_capacitacion:$(this).data('capacitacion'),
                id_empleados:$(this).data('empleado'),
                _token: $('input[name=_token]').val()
            };
            if($(this).is(':checked')){
                data.state = 1
            ...
            ajaxRequest('capacitacion/ABEmpleados',data);
        });
        ...
        $(document).on('click', '.edit', function(){ 
            var id=$(this).data('id');
            $.get('capacitacion/'+id+'/edit', function(data){
                $('#form-button').val(0); 
                $('#modal-title').html('Editar Capacitacion'); 
                $('#modal-form').trigger("reset"); 
                $('#modal').modal('show');
        ...
        /* FIN FORM-BUTTON EDIT */

        /* DELETE BUTTON - LO PONE EN UN ESTADO DE DESACTIVADO */  
        ...
        /* ALTA Y MODIFICACION DE UNA CAPACITACION */
        $("#form-button").click(function(){
            let active = 1;
            if($("#modal-form").valid()){
                $('#form-button').html('Guardando..');
                $.ajax({
                    url:"{{route('capacitacion.AMCapacitacion') }}",
                    method:"POST",
                    data:{
                        id: $("#id").val(),
                        fecha:$('#fecha').val(),
...
@section('content')
    {{-- TABLE INDEX VIEW ALL CAPACITATION ENABLED --}}
                    ...
                    <table style="width:100%" class="table table-striped table-bordered table-hover datatable" id="data-table">
                        <thead>
                            <tr>    
                                ...
                                <th>Lugar</th>
                                <th>Duracion</th>
                                <th style="min-width:30px!important;">Asistentes</th>
                                <th style="min-width:30px!important;">Evaluados</th>
                                <th style="min-width:50px!important;">Accion</th>     
                            </tr>
                        </thead>      
                    </table>                
                </div>
    ...
    {{-- END TABLE INDEX VIEW ALL CAPACITATION ENABLED --}}

    {{-- MODAL - Input fore creation this a new Capacitacion--}}
            ...
            <div class="modal-content">
                ...
                <div class="modal-bod">
                    <form action="post" id="modal-form" name="modal-form" autocomplete="off">
                        ...
                        <div class="form-group row">
                            <label for="nombre" class="col-sm-2 col-form-label">Hora</label>
                            <div class="col-sm">
                                <input type="time" class="timepicker form-control" id="hora" name="hora" placeholder="Hora">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="nombre" class="col-sm-2 col-form-label">Tema</label>
                            <div class="col-sm-10">
                              <input type="text" class="form-control" id="tema" name="tema" placeholder="Tema">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="nombre" class="col-sm-2 col-form-label">Capacitador</label>
                            <div class="col-sm-10">
                              <input type="text" class="form-control" id="capacitador" name="capacitador" placeholder="Capacitador">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="descripcion" class="col-sm-2 col-form-label">Area</label>
                            <div class="col-sm-10">
                              <input type="text" class="form-control" id="area" name="area" placeholder="Area">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="descripcion" class="col-sm-2 col-form-label">Lugar</label>
                            <div class="col-sm-10">
                              <input type="text" class="form-control" id="lugar" name="lugar" placeholder="Lugar">
                            </div>
                        </div>
                        <div class="form-group row">
                            <label for="descripcion" class="col-sm-2 col-form-label">Duracion(min)</label>
                            <div class="col-sm-10">
    ...
    {{-- MODAL - TABLE-PEOPLE --}}
    {{-- Modificar este modal para que la persona pueda observar las personas que estan en la capacitacion--}}
    <div class="modal fade" id="modal-aproved" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-head">
                    <h5 id="modal-title"></h5>
                ...
                <div class="modal-bod">
                    <table style="width:100%" class="table table-striped table-bordered table-hover datatable" id="data-table-aproved">
                        <thead>
                            <tr>    
    ...
    {{-- MODAL - TABLE-ASISTENTES --}}
    <div class="modal fade" id="modal-people" aria-hidden="true">
        <div class="modal-dialog modal-lg">
            <div class="modal-content">
                <div class="modal-head">
                    <h5 id="modal-title"></h5>
                ...
                <div class="modal-bod">
                    <table style="width:100%" class="table table-striped table-bordered table-hover datatable" id="data-table-people">
                        <thead>
                            <tr>
                                <th>DNI</th>
                                <th>Nombre</th>
