...
@section('title', 'Forecast Individual')
@section('post_title', config('app.name'))

@section('content_header')
    <div class="section-header">
        <div>
            <h1><b style="color:#495057;">SIOM</b> Forecast por Variable</h1>
        </div> 
    </div>  
@stop
...
@section('js')
    ...
    <script>
        ...
        var date_fh = moment().endOf('month').format('YYYY-MM-DD');
        $(function () {
        ...
        $(document).ready(function(){ 
            ...
            var table = $('#data-table').DataTable({
                ...
                scrollX : true,
                ajax:{                
                    url: "{{route('forecast_individual.getvalores')}}",
                    type: 'POST',
                    data: function(d){
        ...
        $(document).on('change','.area',function(){
            ...
            $.ajax({
                url:"{{route('historialvariables.getvariables') }}",
                type:"POST",
                dataType: "json",
        ...
        /* ACTION TO CLOSE MODAL */
         ...
         $(document).on('click', '.edit', function(){ 
            var id=$(this).data('id');
            $.get('forecast_individual/'+id+'/edit', function(data){
                if (data['val'] == 1 )
                {
                    $('#form-button').val(0); 
        ...
        $("#form-button").click(function(){
            if($("#modal-form").valid()){
                ...
                $.ajax({
                    url:"{{route('forecast_individual.load') }}",
                    method:"POST",
                    data:{
                        id: $("#id").val(),
                        valor:$('#nvalor').val(),
