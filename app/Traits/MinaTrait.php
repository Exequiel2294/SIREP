    ...
    public function TraitMinaTable(Request $request) {
        ...
        $daypart = (int)date('z', strtotime($this->date)) + 1;
            ...
            //INICIO MES BUDGET
                ...
                //ARMO QUERY DE CONSULTA PARA ONZAS
                    $query="SELECT 10072 as variable_id, SUM( (A.valor *  B.valor ) / 31.1035) AS mes_budget FROM
                    (SELECT fecha, valor
                    FROM [dbo].[budget]
                    where variable_id = 10070) as A
                    INNER JOIN   
            ...
            //INICIO TRIMESTRE BUDGET
                ...
                //INICIO DE QUERY
                    $sql="SELECT 10072 as variable_id, SUM( (A.valor *  B.valor ) / 31.1035) AS trimestre_budget FROM
                    (SELECT fecha, valor
                    FROM [dbo].[budget]
                    where variable_id = 10070) as A
                    INNER JOIN   
            ...
            //INICIO ANIO BUDGET
                ...
                DB::select(
                    'SELECT 10072 as variable_id, SUM( (A.valor *  B.valor ) / 31.1035) AS anio_budget FROM
                    (SELECT fecha, valor
                    FROM [dbo].[budget]
                    where variable_id = 10070) as A
                    INNER JOIN   
