...
class ConciliadoController extends Controller
{
    ...
    public function getvariables(Request $request)
    {
        ...
        if(request()->ajax()) {
            //TOMA EL AREA ID SI ES PLANTA O MINA
            ...
            if ($this->area_id == 1)
            {
                ...
                //INICIO CALCULOS REUTILIZABLES
                    ...
                    DB::select(
                        ...
                        AND fecha between ? and ?
                        GROUP BY variable_id',
                        [date('Y-m-d',strtotime($this->fecha_ini)),date('Y-m-d',strtotime($this->fecha_fin))]
                        );
                ...
                // dd($this->summesreal10005,$this->summesreal10011,$this->summesreal10019,$this->summesreal10031,$this->summesreal10039,
                // $this->summesreal10045,$this->summesreal10052,$this->summesreal10061);
                                ...
                                else
                                {
                                    if (in_array($data->variable_id, $this->promarray))//revisar si variable 10016 corresponde a este grupo
                                    {
                                        ...
                                        $mes_real= DB::select(
                                            ...
                                            AND valor <> 0 
                                            GROUP BY variable_id', 
                                            [$data->variable_id, date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin]
                                        );  
                                        if(isset($mes_real[0]->mes_real))
                                        {
                                            $m_real = $mes_real[0]->mes_real;
            ...
            else
            {
                ...
                //INICIO CALCULOS REUTILIZABLES
                    ...
                    //INICIO MES REAL
                        ...
                        //FIN
                        $this->summesrealonz =
                        DB::select($query,[date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin,
                                            date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin,
                                            date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin,
                                            date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin,
                                            date(('Y-m-d'),strtotime($this->fecha_ini)),$this->fecha_fin,
                        ...
                        //dd($this->summesrealton,$this->summesrealonz,$this->avgmesrealpor);                
                    //FIN MES REAL
                //FIN CALCULOS REUTILIZABLES
...
=======
                ...
                $fechaFin = Periodos::where('anio', $year)
                                ->where('periodo', $month)
                                ->value('fecha_fin');
                $this->fecha_ini = date('Y-m-d',strtotime($fechaIni));
                $this->fecha_fin = date('Y-m-d',strtotime($fechaFin));
